...
F040 — PUBLIC Service Request Staff Workspace implements the approved granular PUBLIC permission model. One staff list/detail uses a server-authorized PUBLIC/INTERNAL union, per-request capabilities, shared lifecycle/routing/ownership/history and the independent F039 contact boundary. The two new keys are `service_request.route` and `service_request.watchers.manage`; no default grant is introduced. FULL_UAT_OPERATOR's explicitly approved F040 expansion was sixteen keys. Automated regressions and manual authenticated PUBLIC operations, mixed filters, read/operation separation and independent audience/contact revocations passed. See [F040](features/F040-public-service-request-staff-workspace.md) and its [implementation report](features/F040-implementation-report.md). F041 adds explicit Notes read/create permissions and immutable staff-only collaboration on both audiences, with no default grants. The tested personal migration is applied (20 total, zero pending); incremental explicit provisioning and manual creation/revocation/privacy UAT passed. Eighteen keys are retained and two fictional Notes exist, without parent operational changes. All three responsive/theme states and keyboard checks passed. See [F041](features/F041-internal-notes-staff-collaboration.md) and its [implementation report](features/F041-implementation-report.md) for final validation status. F042 remains unselected and unstarted. The following F039 record describes the accepted starting checkpoint.
...
F038 establishes a reusable visual language for Reqro, the current client-neutral platform direction, while retaining CityVUE application/repository naming. The approved Service Request detail concept informs an Issue-first staff workspace with real configured icons/location, shared semantic status/activity presentation, responsive navigation and light/dark tokens. Existing fictional data and F029–F037 security behavior remain intact. No production logo adoption or historical rename occurred. Older surface normalization is explicitly partial; see [F038](features/F038-unified-ui-design-system-experience-refresh.md).
