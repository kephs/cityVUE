...
F056.2B intentionally tightens both legacy F032 and modern handling writes to require `admin.configuration.read`, `admin.issues.write` and `catalog.issue_action.manage`, retaining Organization and Department/Division scope. Action-only grants retain the established scoped read contract but no longer authorize mutation. Do not expand provisioning bundles to conceal this compatibility change. Availability changes through the governed atomic Issue configuration command affect future intake only; historical requests are not reclassified (see [ADR-022](../architecture/decisions/ADR-022-governed-issue-availability.md)); PUBLIC means external intake context and authorized INTERNAL means internal context, independently of channel. See [ADR-019](../architecture/decisions/ADR-019-issue-availability-governed-handoff.md).

F056.2A submitted Dynamic Question answers require current parent Service Request access plus `service_request.answers.read`; ordinary legacy detail no longer implies disclosure. Both supported staff consumers use the protected no-store path with audit-before-disclosure. Preserve published children and immutable answer snapshots; current schema changes must not rewrite history. See [ADR-018](../architecture/decisions/ADR-018-dynamic-questions-protected-answers.md). Permission registration and CLI allowlisting are not grants; no broad development bundle includes answers.read.
