...
## Audience/reference spacing follow-up — 2026-09-22

From accepted `c81edb96d349eeff8c0527c38819d37044eb8687`, a separate presentation-only refinement groups the detail header's Audience badge and Request # in a wrapping flex row, with existing `space-2`/`space-4` gaps. It preserves Issue → Service Location → audience/reference order, secondary reference styling, semantic labels and the separate status area. No API, authorization, database or grant changes. Focused workspace/design-system tests: **151 passed**; frontend build passed with the existing large-chunk warning. Synthetic browser layout checks passed all **24** combinations of 1280/768/390, light/dark, PUBLIC/INTERNAL and normal/long references without overlap, touching, clipping or horizontal overflow. This is fixture-based visual validation, not live authenticated UAT or accessibility certification. No push, deployment or F046 work.

## Staff list controls refinement — 2026-09-22

Starting checkpoint: `2dd908f40bb89311663c3c943c63dc8130c395fb`, clean `main`, two ahead/zero behind cached `origin/main`. This separately committed enhancement preserves both earlier UI commits. It is not F046 and authorizes neither push nor deployment.

### Sorting contract

| UI column             | API sort key | Database/domain meaning                                                                         | Tie-breaker             |
| --------------------- | ------------ | ----------------------------------------------------------------------------------------------- | ----------------------- |
| Issue / Reference     | `issue`      | Published/versioned Issue display name, then persisted Request Reference; both follow direction | Request UUID descending |
| Status                | `status`     | Canonical persisted status code, lexical database order (not lifecycle priority)                | Request UUID descending |
| Department / Division | `department` | Effective routed Department name, then effective Division name; both follow direction           | Request UUID descending |
| Assignment            | `assignment` | Existing safe current-owner display name, then supported type; both follow direction            | Request UUID descending |
| Created               | `created`    | Persisted creation timestamp                                                                    | Request UUID descending |

Accepted directions: `asc`, `desc`. Default: `created` / `desc`, preserving the prior timestamp-descending/UUID-descending order. Division NULLs and unassigned owner NULLs sort **last in both directions**; other listed primary fields are non-null. Text ordering follows the database collation. Issue/Reference is one composite sort, not separate independent Issue and Reference toggles. Assignment uses the same sanitized display projection as the existing list, not private identity fields or a new directory endpoint. Inactive retained current targets remain owned until explicitly removed, matching F037; this task does not perform offboarding.

DTO allowlists reject unsupported sort keys, direction or assignment values with HTTP 400. Repository validation also rejects unsupported direct inputs. **The browser cannot supply an arbitrary SQL ORDER-BY expression.** SQL expressions/column names are fixed by the server; values remain parameterized. Assignment filter accepts only `all`, `assigned`, `unassigned`.

### Authorization, filters and pagination

After query validation, the existing query resolves trusted Organization, active membership/audience read permission and effective Department/Division request scope. Audience selection, My Requests/My Team/Watching, exact Request Reference, status and routing filters only narrow that set. Assignment filtering is applied to the same filtered query used for totals. Fixed sorting applies to the authorized projection **before LIMIT/OFFSET**. No current-page React sort or filter exists. Neither sorting nor ownership/membership expands the authorized set. Contact, Notes, Communication bodies and other protected fields are absent from the list projection.

Assigned means the existing current assignment projection resolves a supported STAFF, operational ROLE or GROUP/TEAM target in the same Organization, with `ended_at IS NULL`. Unassigned means that projection is absent: no current supported F037 owner relationship. Historical/ended/removed assignments do not count; Watchers and Department/Division routing never count. Retained historical legacy Department-only relationships are not F037 owners. No new grant or permission is introduced.

Default work queue remains Audience **All**, view **All Requests**, Assignment **All assignments**, Created **descending**, page size **25**. Sort/direction and applied filters persist through pagination; sort/filter changes reset page to 1. Reset restores all defaults. Existing explicit Apply filters semantics remain; no Live Search was added. Labeled Sort by/Direction selectors remain available for mobile cards, while desktop column buttons expose sortable state, arrows, descriptive names and `aria-sort`. Hidden mobile header buttons are not keyboard stops. The new audience/reference flex metadata reuses the approved detail spacing and supports long references.

Row # is the presentation ordinal `((page - 1) * pageSize) + rowIndex + 1`: page 1 at size 25 is 1–25; page 2 is 26–50. Sorting/filtering changes positions. It is not a request UUID, Request Reference, database/creation sequence or permanent identifier, and **no row number is persisted**. Table caption and per-row accessible labels identify the position; Request Reference remains the human identifier.

### Validation and review

Disposable PostgreSQL fixtures create 28 additional synthetic requests with timestamp ties, both audiences, differing statuses and current/removed owner relationships. Tests verify all five sorts in both directions, descending UUID ties, NULL-last behavior, concatenated pages against the global result, totals, query validation, assigned/unassigned partitions and safe projection. Examples: PUBLIC/INTERNAL plus Unassigned plus Created descending; Open plus fictional Department plus Unassigned plus Issue ascending; My Team plus Assigned plus Created descending. My Requests/My Team/Watching combinations retain the same independently authorized set; denied principals remain denied. Removing ownership while retaining a Watcher still yields Unassigned. No personal development data is changed to manufacture fixtures.

Automated results: backend unit **212 passed**; PostgreSQL **175 passed, zero skips**; API E2E **40 passed**. Full React suite **432 passed**; final affected React/design/repository run **164 passed** after mobile label grouping; shared suite **62 passed**. TypeScript, backend lint, configured backend formatting, changed frontend/document formatting, backend/frontend production builds, Git whitespace, documentation links and scoped private-data/artifact review passed. No frontend lint script exists. The normal production frontend build retains its existing >500 kB chunk warning. The first database attempt exposed an incorrect expectation that an already denied principal could list; the fixture now asserts continued denial, with no authorization change.

Browser validation uses the actual list component with synthetic repository responses, not an authenticated session. All 1280/1024/768/390 light/dark combinations had usable controls/cards, separated audience/reference, visible row numbers/assignment, and no page overflow. Keyboard header activation toggled every sort direction; labeled selectors provide mobile sorting, Assigned/Unassigned narrowed synthetic results, Reset restored defaults and page 2 began at 26. Visual inspection includes desktop light/mobile dark and long-reference cards. Visible focus and accessible names/direction/row context are checked; no WCAG or physical-device certification is claimed. The operator reported **Passed all available read-only checks** in the existing signed-in local staff session: audience/reference spacing and row numbers, all five headers in both directions, Assigned/Unassigned composed with available Audience/Status filters, and Reset defaults. This is operator-reported live evidence, not independently observed browser automation. Page-two and broad dataset composition remain disposable/browser-fixture evidence where the live dataset does not support them. No request or assignment mutation was requested for UAT.

Security review preserves Organization and PUBLIC/INTERNAL authorization, workspace view membership narrowing, protected projections and all mutation boundaries. Existing ownership projection indexes are reused; no new schema is required. Assignment sorting/filtering uses correlated existing projections; production-scale query-plan benchmarking is not claimed. Offset pagination remains stable for equal primary values using UUID ties, but does not promise a frozen snapshot during concurrent data changes. The 12-file scope comprises this F043 record, the F040 record and roadmap; InternalRequestWorkspace.jsx, requestRepository.js and staffRequests.css; the staff query DTO/controller, internal-request repository and new staff-list-controls validator; plus staff workspace React, database workspace and validator unit tests. No screenshots/logs/UAT fixture files are committed. No migration, database/grant change, credential lifecycle operation, deployment or cloud/client resource change is part of this task.
