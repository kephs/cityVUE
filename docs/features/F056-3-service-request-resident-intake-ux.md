...
Post-F056.3 filter-bar refinement: the separate Reference textbox is removed. Search Requests remains the normal reference/Issue/Service Location lookup. Legacy exact-reference `search` URLs still constrain the existing backend query, compose with other filters and survive navigation; Reset clears them along with the existing filter/search/page state. There is no new active-filter chip system. At desktop widths above 1100px the filter card has two rows: Audience / Request View / Assignment, then Status / Department / Division / Reset + Apply Filters. The action group is right-aligned and bottom-aligned with dropdowns, with Apply Filters primary. At 1100px and below each row reflows into two columns; at 480px and below controls stack while the two actions remain grouped. Search remains a separate blue-accented surface. Its helper is now “Results update as you type.”; associated labeling and validation feedback remain. Audience and Request View retain their existing immediate application; other filters retain Apply Filters behavior. No query, scope, authorization or backend contract changes.
