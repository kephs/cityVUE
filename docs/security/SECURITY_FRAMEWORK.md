...
| Structured Pino logs, selected field redaction, correlation IDs | [logger](../../server/src/common/logging/pino-logger.service.ts), [request middleware](../../server/src/common/logging/request-logging.middleware.ts)                                                                                                                    | Safe-format inbound IDs are retained; otherwise UUIDs are generated. Redaction paths do not guarantee sanitation of arbitrary messages, URLs, nested values, or serialized errors.                                                                                                       |
| Database transport safeguard and API persistence boundary | [environment.ts](../../server/src/config/environment.ts), [database service](../../server/src/database/database.service.ts) | Production intends verified TLS, but currently only rejects the explicit disabled mode. Unverified require mode and connection-string TLS overrides mean effective certificate verification is not guaranteed. |
...
**Known risk — not yet hardened in this baseline:** Production intends encrypted transport with authenticated server certificates, but current startup validation rejects only DATABASE_SSL_MODE=disable and still permits unverified require mode. PostgreSQL URL TLS options can override explicit application settings, including verify-full, so effective TLS and certificate verification are not guaranteed. API and migration tooling duplicate this decision. A later implementation must enforce one shared policy, reject overrides, support approved CA trust, and test effective driver configuration. Production handshakes and HTTPS ingress remain separate validation work.
...
Citizen identity/tracking, final City role grants, enterprise adapters, managed secrets, full security audit/monitoring, incident/recovery procedures, production transport/isolation validation, and production identity UAT remain separately scoped work. Resolve the database TLS configuration-precedence risk and logging limitations, then validate deployed certificate trust during that work. No City-wide policy, certification, compliance assessment, or completed production security posture is asserted.
