...
### Controlled Access Reader amendment discovered during F057.2 UAT preparation

Access Reader authority is distinct from Access Administrator authority. The existing personal development identity already had configuration-read, but neither access permission; manager bootstrap would overgrant authority and establish a lasting bootstrap invariant. Explicitly approved Migration 39 adds `reader` to existing single-principal provisioning ownership and `provision_access_reader` / `revoke_access_reader` to the constrained audit vocabulary. A separate ownership kind is necessary because the existing unique principal/kind relationship must allow Reader and Administrator contributions to coexist and be independently removed. It creates no tables, permission registrations, grants, bootstrap state or historical events. Migrations 1–38 remain unchanged.

Controlled `grant-reader` / `remove-reader` operations change only their dedicated `admin.access.read` contribution. They never grant configuration-read or access-manage, establish bootstrap, or adopt other roles. Configuration-read remains independently required. Existing transaction locking, expected revision/bootstrap state, one revision per actual transaction, typed provisioning audit and immutable deltas apply. Dry-runs and no-ops write nothing. Empty ownership/assignment and history remain after removal. Independent existing read authority survives removal and is reported. Reader authority can complete an independently supplied manager permission set; existing effective last-manager checks still apply, including removal previews. Administrator operations are unchanged.

Migration 39 adds Reader-specific audit ownership/direction and permission integrity checks. Down refuses any retained Reader ownership or Reader audit, including emptied roles; before use, down/reapply is safe. No session bypass or direct-table provisioning path is introduced.

The local CLI retains synthetic-only manager provisioning. Reader operations additionally accept explicit personal-development UAT selection (`F057_PERSONAL_READER_UAT=true`) with personal tenant opt-in, an existing active mapped staff target, stable internal IDs, expected state and the existing localhost database restrictions. This does not authorize production/client provisioning or Entra changes. Actual personal Reader grant remains subject to review of its dry-run. See the [Reader prerequisite report](../../features/F057-access-reader-prerequisite.md).
