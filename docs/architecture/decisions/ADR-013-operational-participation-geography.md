...
## Approved amendment — Organization collection control (2026-09-23)

The separately authorized F051 follow-up adds `organization.service_participation_collection_enabled`, a non-null boolean defaulting to false. This supersedes the original assumption that available areas alone are enough to offer collection. The original decision above is retained as historical context. No generic feature-flag platform or production administration API/UI is introduced.

The migration enables no Organization and rewrites no request. Only the already-approved fictional personal-development Organization is explicitly enabled using the guarded F051 CLI after migration. New and unrelated Organizations remain disabled. Existing areas never imply consent to collect; default provisioning creates areas without enabling collection.

The safe intake response is `{collectionEnabled, items}`. Disabled returns false and no items; enabled/ready returns true plus active safe id/label choices; enabled/incomplete returns true and no items. The frontend hides the entire section during disabled/incomplete/unavailable collection, clears stale selections on resolution, omits Review geography and submits without geography. No empty selector, technical configuration warning, orphan label or tab stop remains. The CLI `status` exposes disabled/ready/incomplete plus active-area count only to the local operator. Enabled/incomplete does not block ordinary submission; it rejects supplied PROVIDED or DECLINED because no valid collection question is available.

Creation with explicit geography locks the Organization setting for share in the same transaction as area validation and insertion. A configuration update serializes with that check. Disabled collection rejects both PROVIDED and DECLINED on public, trusted API and assisted PUBLIC paths. Omitted input remains NOT_COLLECTED; INTERNAL participation still fails. Already-finalized evidence retries return the original immutable receipt before current policy checks. Stale browsers may receive a safe rejection after policy changes and must refresh; data is not silently retained contrary to policy.

Toggling only changes future collection. It does not delete/deactivate areas, change historical PROVIDED/DECLINED/NOT_COLLECTED, backfill disabled-period requests, affect Contact/identity/Service Location/assignment, or grant analytics access. Historical analytics continues under the unchanged dedicated permission, PUBLIC scope, threshold and suppression rules. Requester History, Live Search and tracking remain unchanged.

The development CLI supports `enable --dry-run/--confirm`, `disable --dry-run/--confirm`, and read-only `status`, with the existing explicit development/fictional-data opt-in and target checks. Repeating a setting is idempotent. No mutation route or broad permission is added. Production configuration mutation authorization/audit remains deferred.

Future Admin Portal: Settings → Service Requests → Intake Settings → Service Participation → **Collect service participation information [On/Off]**. Suggested explanation: “When enabled, requesters may optionally select an approved participation area or choose Prefer not to say. When disabled, Reqro does not ask for this information during Service Request intake.” This control does not delete historical data. Area add/rename/reorder/deactivate/governance UI and threshold administration remain deferred; any future threshold control must preserve the hard minimum.
