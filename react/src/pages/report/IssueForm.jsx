...
export default function IssueForm({ service, visibleQuestions, values, answers, errors, onValueChange, onAnswerChange, onContinue, onBack, locationRepository, onLocationChange }) {
    ...
    return (
        <form noValidate onSubmit={(event) => { event.preventDefault(); onContinue(); }}>
            ...
            {onLocationChange && service.locationRequirement !== "not-applicable" ? <ServiceLocationInput repository={locationRepository} text={values.location} point={values.locationPoint} onChange={onLocationChange} required={service.locationRequirement === "required"} geographicPolicy={service.geographicEligibilityMode} error={errors.location} /> : service.locationRequirement === "required" && <div className="mb-4"><label className="form-label" htmlFor="location">Location <span aria-hidden="true">*</span></label><input {...fieldProps("location")} type="text" placeholder="Enter the issue location" required /><FieldError id="location-error" message={errors.location} /></div>}
            <fieldset className="mb-4"><legend className="h5">How would you like to report this concern?</legend><div className="form-check mb-2"><input className="form-check-input" id="report-anonymous" type="radio" name="reportingMode" value="anonymous" checked={!identified} disabled={service.anonymousPolicy === "not-allowed"} onChange={(event) => onValueChange("reportingMode", event.target.value)} /><label className="form-check-label" htmlFor="report-anonymous">Report anonymously</label></div><div className="form-check"><input className="form-check-input" id="report-identified" type="radio" name="reportingMode" value="identified" checked={identified} onChange={(event) => onValueChange("reportingMode", event.target.value)} /><label className="form-check-label" htmlFor="report-identified">Provide my name</label></div>{service.anonymousPolicy === "not-allowed" && <p className="form-text">This issue requires a reporter name.</p>}</fieldset>
