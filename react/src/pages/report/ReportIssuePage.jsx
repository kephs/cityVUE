...
const stepLabels = { service: "Choose a service", details: "Request details", review: "Review your request" };
...
function serviceCountText(count) {
    return `${count} ${count === 1 ? "service" : "services"} found`;
}
    ...
    return (
        <section className="report-issue-page" aria-labelledby="report-heading">
            <div className="intake-header">
                <header className="report-intro"><span className="report-intro-icon" aria-hidden="true"><i className="bi bi-megaphone" /></span><div><h1 id="report-heading">Report an Issue</h1><p>Find the service that best matches your concern.</p></div></header>
            ...
            <div className="card border-0 report-form-card"><div className="card-body p-3 p-md-4 p-lg-5">
                {step === "service" && <div className="step-panel">
                    ...
                    <div className="prototype-notice"><i className="bi bi-info-circle" aria-hidden="true" /><span>Prototype data — sample service catalog for development.</span></div>
                    ...
                    {category && <section className="service-section" aria-labelledby="service-heading"><div className="section-heading compact"><span className="section-symbol" aria-hidden="true"><i className="bi bi-search" /></span><div><h2 className="h4" id="service-heading">Choose a Service</h2><p>Showing services in <strong>{category.name}</strong>.</p></div></div><label className="form-label fw-semibold" htmlFor="service-search">Search services</label><div className="service-search"><i className="bi bi-search" aria-hidden="true" /><input className="form-control" id="service-search" type="search" placeholder={`Search services in ${category.name}`} value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} />{searchQuery && <button className="btn btn-outline-secondary" type="button" onClick={() => setSearchQuery("")}>Clear<span className="visually-hidden"> service search</span></button>}</div><p className="result-count" aria-live="polite">{serviceCountText(services.length)}</p>{services.length ? <div className="service-list" role="radiogroup" aria-label="Service">{services.map((item) => <label key={item.id} className={`catalog-choice service-choice${serviceId === item.id ? " selected" : ""}`}><input type="radio" name="service" value={item.id} checked={serviceId === item.id} onChange={() => selectService(item.id)} /><span className="choice-copy"><strong>{item.name}</strong><small>{item.citizenDescription}</small></span><i className={`bi ${serviceId === item.id ? "bi-check-circle-fill" : "bi-chevron-right"} choice-state`} aria-hidden="true" /></label>)}</div> : <div className="empty-services" role="status"><i className="bi bi-search" aria-hidden="true" /><h3>No matching services found.</h3><p>Try a different word or clear your search.</p><button className="btn btn-outline-primary" type="button" onClick={() => setSearchQuery("")}>Clear search</button></div>}<aside className="service-guidance"><strong>Can't find what you're looking for?</strong><span>Try another search or choose a different category.</span></aside>{errors.service && <div className="text-danger mt-2" role="alert">{errors.service}</div>}</section>}
                ...
                {step === "details" && service && <div className="step-panel"><div className="selected-service-banner"><i className="bi bi-check-circle" aria-hidden="true" /><div><span>Selected service</span><p className="service-path">{category?.name} <i className="bi bi-chevron-right" aria-hidden="true" /> Service details</p><h2 className="h4" tabIndex="-1" data-step-heading>{service.name}</h2><p>{service.citizenDescription}</p></div></div><div className="details-surface"><IssueForm service={service} visibleQuestions={visibleQuestions} values={values} answers={answers} errors={errors} onValueChange={changeValue} onAnswerChange={changeAnswer} onContinue={continueFromDetails} onBack={() => moveToStep("service")} /></div></div>}
                {step === "review" && service && category && <div className="step-panel"><div className="review-heading"><span className="review-icon" aria-hidden="true"><i className="bi bi-clipboard-check" /></span><div><h2 className="h4" tabIndex="-1" data-step-heading>Review Your Request</h2><p>Please review your request before submitting.</p></div></div><dl className="review-list"><div className="review-pair"><dt>Selected Service</dt><dd><strong>{service.name}</strong><span>{category.name}</span></dd></div><div className="review-pair"><dt>Location</dt><dd>{values.location}</dd></div><div className="review-pair"><dt>Reporting Information</dt><dd>{values.reportingMode === "anonymous" ? "Reporting anonymously" : values.reporterName}</dd></div><div className="review-pair"><dt>Details</dt><dd>{values.description}</dd></div>{visibleQuestions.filter((question) => answers[question.id]).map((question) => <div key={question.id} className="review-pair"><dt>{question.label}</dt><dd>{answers[question.id]}</dd></div>)}</dl><div className="intake-actions action-footer"><button className="btn btn-outline-secondary" type="button" onClick={() => moveToStep("details")}><i className="bi bi-arrow-left me-2" aria-hidden="true" />Back / Edit</button><button className="btn btn-primary" type="button" disabled={isSubmitting} onClick={submitRequest}>{isSubmitting ? "Submitting..." : "Submit Request"}<i className="bi bi-send ms-2" aria-hidden="true" /></button></div></div>}
            </div></div>
        </section>
    );
}
