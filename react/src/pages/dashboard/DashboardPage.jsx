...
import { getDepartmentCounts } from "./departmentStatistics.js";
...
export default function DashboardPage({ loadIssues = () => IssueService.getIssues() }) {
    ...
    const departmentCounts = getDepartmentCounts(issues);
    ...
    return <section className="dashboard-page" aria-labelledby="dashboard-heading">
        ...
        <section className="dashboard-panel dashboard-department-panel mb-4" aria-labelledby="department-chart-heading"><div className="dashboard-panel-header"><div><h2 id="department-chart-heading"><i className="bi bi-building me-2" aria-hidden="true" />Issues by Department</h2><p>Current issues grouped by owning Department.</p></div></div><div className="dashboard-department-layout"><div className="dashboard-department-chart" style={{ height: `${Math.max(220, departmentCounts.length * 48 + 70)}px` }}><DashboardChart type="bar" indexAxis="y" labels={departmentCounts.map(({ department }) => department)} values={departmentCounts.map(({ count }) => count)} label="Number of issues" /></div><dl className="dashboard-department-list" aria-label="Department chart data">{departmentCounts.map(({ department, count }) => <div key={department}><dt>{department}</dt><dd aria-label={`${count} ${count === 1 ? "issue" : "issues"}`}>{count}</dd></div>)}</dl></div></section>
        <section className="dashboard-panel dashboard-recent-panel" aria-labelledby="recent-issues-heading"><div className="dashboard-panel-header"><h2 id="recent-issues-heading"><i className="bi bi-clock-history me-2" aria-hidden="true" />Recent Issues</h2><Link className="dashboard-view-all" to="/issues">View All Issues<i className="bi bi-chevron-right" aria-hidden="true" /></Link></div><ul className="dashboard-recent-list">{recent.map((issue, index) => <li key={issue.id || `recent-${index}`}><Link className="dashboard-recent-item" to={`/issues/${encodeURIComponent(String(issue.id || ""))}/edit`}><span className="dashboard-recent-icon" aria-hidden="true"><i className={`bi ${getIssueIcon(issue)}`} /></span><span className="dashboard-recent-copy"><strong>{issue.title || "Untitled Issue"}</strong><span>{issue.category || "Uncategorized"} · {formatIssueDate(issue.dateReported) || "Date unavailable"}</span></span><StatusBadge value={issue.status || "Open"} /><i className="bi bi-chevron-right dashboard-recent-arrow" aria-hidden="true" /></Link></li>)}</ul></section>
    </section>;
}
