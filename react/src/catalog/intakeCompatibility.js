...
export function formatCompatibilityDescription({
  service,
  answers,
  description,
}) {
  ...
  const answerLines = getVisibleQuestions(service, answers)
    ...
    .map(
      (question) =>
        `- ${question.label}: ${formatAnswer(question, answers[question.id])}`,
    );
...
export function mapIntakeToLegacyIssue({
  category,
  service,
  answers,
  description,
  ...
  return {
    title: service.name,
    description: formatCompatibilityDescription({
      service,
      answers: {},
      description,
    }),
    ...
    status: "Open",
    reportedBy:
      reportingMode === "anonymous" ? "Anonymous" : reporterName.trim(),
