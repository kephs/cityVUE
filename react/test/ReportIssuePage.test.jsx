...
function renderPage(props = {}) {
  return render(
    <MemoryRouter initialEntries={["/report"]}>
      <Routes>
        <Route path="/report" element={<ReportIssuePage {...props} />} />
        <Route
          path="/issues"
          element={<div role="status">Issue submitted successfully.</div>}
        />
      </Routes>
    </MemoryRouter>,
  );
...
describe("ReportIssuePage configuration-driven intake", () => {
  test("renders active fixture Categories without exposing Departments", () => {
    ...
    expect(
      screen.getByRole("radio", { name: /Trash & Recycling/ }),
    ).toBeInTheDocument();
    expect(screen.queryByText("Public Works")).not.toBeInTheDocument();
    expect(
      screen.queryByText("Environmental Services"),
  ...
  test("Category live search filters names and descriptions with singular and plural counts", async () => {
    ...
    expect(
      screen.getByRole("radio", { name: /Streetlights/ }),
    ).toBeInTheDocument();
    await user.clear(search);
    await user.type(search, "standing water");
    expect(screen.getByText("1 category found")).toBeInTheDocument();
    expect(
      screen.getByRole("radio", { name: /Water & Drainage/ }),
    ).toBeInTheDocument();
    expect(
  ...
  test("Category zero-result state clears search, restores Categories, and returns focus", async () => {
    ...
    expect(
      screen.getByRole("heading", { name: "No matching categories found." }),
    ).toBeInTheDocument();
    expect(
      screen.getByText("Try a different word or clear your search."),
    ).toBeInTheDocument();
  ...
  test("Category selection reveals Issues, renders configured icons, and changing Category clears selection", async () => {
    ...
    renderPage();
    expect(
      screen.queryByRole("heading", { name: "Choose an Issue" }),
    ).not.toBeInTheDocument();
    await selectPothole(user);
    expect(
      screen.getByRole("heading", { name: "Choose an Issue" }),
    ...
    expect(
      screen.queryByRole("heading", { name: "Choose a Service" }),
    ).not.toBeInTheDocument();
    expect(screen.getByText("2 issues found")).toBeInTheDocument();
    expect(
      screen
        .getByRole("radio", { name: /Pothole/ })
        .closest("label")
        .querySelector(".bi-cone-striped"),
    ).toBeInTheDocument();
    expect(
  ...
  test("live search supports aliases and a clear zero-result state", async () => {
    ...
    expect(search).toHaveAttribute(
      "placeholder",
      "Search issues in Roads & Streets",
    );
    await user.type(search, "hole in road");
    expect(screen.getByText("1 issue found")).toBeInTheDocument();
    expect(screen.getByRole("radio", { name: /Pothole/ })).toBeInTheDocument();
    expect(
      screen.queryByRole("radio", { name: /Damaged Street Sign/ }),
    ).not.toBeInTheDocument();
    await user.clear(search);
    await user.type(search, "no such service");
    expect(
      screen.getByRole("heading", { name: "No matching issues found." }),
    ).toBeInTheDocument();
    expect(
      screen.getByText("Try a different word or choose another category."),
    ).toBeInTheDocument();
  });
  ...
  test("mobile Category changes focus and reveal the Issue section with reduced-motion respected", async () => {
    ...
    renderPage();
    await user.click(
      screen.getByRole("radio", { name: /Trees & Landscaping/ }),
    );
    const heading = await screen.findByRole("heading", {
      name: "Choose an Issue",
    });
    ...
    expect(
      screen
        .getByRole("radio", { name: /Fallen Tree or Branch/ })
        .closest("label")
        .querySelector(".bi-tree"),
    ).toBeInTheDocument();
    Object.defineProperty(window, "innerWidth", {
      configurable: true,
      value: originalWidth,
  ...
  test("F049 requires an explicit choice and erases Contact when switching to anonymous", async () => {
    const user = userEvent.setup();
    renderPage();
    await reachPotholeDetails(user);
    expect(
      screen.getByRole("radio", { name: "Report anonymously" }),
    ).not.toBeChecked();
    expect(
    ...
    await user.click(screen.getByRole("radio", { name: "Provide my name" }));
    await user.type(screen.getByLabelText("Your name *"), "Disposable Contact");
    await user.click(screen.getByRole("radio", { name: "Report anonymously" }));
    expect(screen.queryByLabelText("Your name *")).not.toBeInTheDocument();
    await user.click(screen.getByRole("radio", { name: "Provide my name" }));
    ...
    expect(screen.queryByText("Disposable Contact")).not.toBeInTheDocument();
  });
  ...
  test("review summarizes values and Back preserves relevant state", async () => {
    ...
    renderPage();
    await reachPotholeDetails(user);
    await completeAnonymousDetails(user, "yes");
    await user.click(screen.getByRole("button", { name: "Review request" }));
    expect(
    ...
    expect(
      screen.getByText("Large pothole near the intersection."),
    ).toBeInTheDocument();
    expect(screen.getByText("Reporting anonymously")).toBeInTheDocument();
  ...
  test("submits the exact compatibility payload once and navigates successfully", async () => {
    const user = userEvent.setup();
    const saveIssue = vi.fn();
    renderPage({
      saveIssue,
      createTimestamp: () => "2026-08-29T18:00:00.000Z",
    });
    await reachPotholeDetails(user);
    await completeAnonymousDetails(user);
    await user.click(screen.getByRole("button", { name: "Review request" }));
    ...
    expect(saveIssue).toHaveBeenCalledWith({
      ...
      dateReported: "2026-08-29T18:00:00.000Z",
    });
  ...
  test("save failure preserves review state and allows retry", async () => {
    ...
    const consoleError = vi
      .spyOn(console, "error")
      .mockImplementation(() => {});
    const saveIssue = vi.fn().mockImplementationOnce(() => {
      throw new Error("quota");
    });
    renderPage({ saveIssue });
    await reachPotholeDetails(user);
    await completeAnonymousDetails(user);
    await user.click(screen.getByRole("button", { name: "Review request" }));
