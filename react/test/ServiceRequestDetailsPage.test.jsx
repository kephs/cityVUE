...
const details = {
  ...
  classification: {
    issueName: "Pothole",
    category: { id: "category", name: "Roads & Streets" },
    department: { id: "department", name: "Public Works" },
    division: { id: "division", name: "Streets" },
  },
...
function renderPage(data) {
  return render(
    <MemoryRouter
      initialEntries={["/issues/80000000-0000-4000-8000-000000000001"]}
    >
      <Routes>
        <Route
          path="/issues/:issueId"
          element={<ServiceRequestDetailsPage data={data} />}
        />
      </Routes>
    </MemoryRouter>,
  );
...
test("renders the complete canonical read model with persisted answer snapshots", async () => {
  renderPage({
    mode: "api",
    repository: { getServiceRequestDetails: vi.fn(async () => details) },
  });
  expect(
...
test("renders anonymous requester and omits optional sections safely", async () => {
  const minimal = {
    ...details,
    classification: { ...details.classification, division: undefined },
    requester: { anonymous: true },
    location: undefined,
  };
  renderPage({
    mode: "api",
    repository: { getServiceRequestDetails: vi.fn(async () => minimal) },
  });
...
test("shows loading, not-found, and retryable generic error states", async () => {
  ...
  await screen.findByRole("heading", {
    name: "Request details could not be loaded",
  });
...
test("keeps canonical details unavailable outside explicitly enabled API mode", () => {
  ...
  expect(
    screen.getByRole("heading", { name: "Request details unavailable" }),
  ).toBeInTheDocument();
});

test("legacy details fetch answers only through protected read when authorized", async () => {
  const readAnswers = vi.fn(async () => ({
    answers: [
      {
        questionId: "q",
        label: "Historical question",
        displayValue: "Historical response",
      },
    ],
  }));
  renderPage({
    mode: "api",
    repository: {
      getServiceRequestDetails: vi.fn(async () => ({
        ...details,
        canReadAnswers: true,
      })),
      readAnswers,
