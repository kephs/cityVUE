...
const projectCommunication = (row: CommunicationRow) => ({
  id: row.id,
  body: row.body,
...
export function communicationCursor(
  value?: string,
  ...
  try {
    ...
    throw new BadRequestException('Invalid communications cursor');
  }
}

/** No update/delete operations. Every caller must authorize the parent within this transaction. */
@Injectable()
export class RequestCommunicationRepository {
  async list(
    ...
    if (!Number.isInteger(pageSize) || pageSize < 1 || pageSize > 100)
      throw new BadRequestException('Invalid communications page size');
    const before = communicationCursor(cursor);
    let query = trx
    ...
    const items = rows.slice(0, pageSize).map(projectCommunication);
  ...
  async create(
    ...
    const row = await trx
      ...
      .executeTakeFirst();
    if (row) return { communication: projectCommunication(row), created: true };
    ...
    if (prior.body !== input.body)
      throw new ConflictException(
        'Message submission changed; reload before retrying',
      );
    return { communication: projectCommunication(prior), created: false };
  }
}
